...
namespace CF.Web.AspNetCore.Filters
{
    ...
    {
        private const string RedirectMessagesTempDataKey = "cfweb-redirect-messages";
            ...
            { typeof(RedirectToActionResult), x =>
                {
                    ...
                    return result.UrlHelper.Action(result.ActionName, result.ControllerName, result.RouteValues, protocol: null, host: null, fragment: result.Fragment);
                }
            },
            { typeof(RedirectToPageResult), x =>
                {
                    var result = x as RedirectToPageResult;
                    return result.UrlHelper.Page(result.PageName, result.PageHandler, result.RouteValues, protocol: null, host: null, fragment: result.Fragment);
                }
            },
            { typeof(RedirectToRouteResult), x =>
                {
                    var result = x as RedirectToRouteResult;
                    return result.UrlHelper.RouteUrl(result.RouteName, result.RouteValues, protocol: null, host: null, fragment: result.Fragment);
                }
            },
        };
        ...
        public RedirectMessageActionFilter(
            IWebHostEnvironment env, IScopedRedirectMessageRecorder redirectMessageRecorder, ILogger<RedirectMessageActionFilter> logger)
        {
        ...
        public void OnActionExecuted(ActionExecutedContext context)
        {
                ...
                this._redirectMessageRecorder.OutboundMessages.Any())
            {
                ...
                {
                    redirectUri = UrlFuncByRedirectType[context.Result.GetType()](context.Result);
                }
                else if ((context.Result as JsonResult)?.Value is AjaxActionResultPackage ajaxActionResultPackage && ajaxActionResultPackage.RedirectUri != null)
                {
                    redirectUri = ((AjaxActionResultPackage)((JsonResult)context.Result).Value).RedirectUri?.ToString();
                }
                ...
                {
                    targetUrlPathAndQuery = new Uri(redirectUri, UriKind.Absolute).PathAndQuery;
                }
                else
                {
                    targetUrlPathAndQuery = redirectUri;
                }
            ...
            if (webController != null && (webController.TempData[RedirectMessagesTempDataKey] as string) != null)
            {
                ...
                try
                {
                    ...
                    if (redirectMessagesModel != null)
                    {
                        ...
                        // The referer is a nice-to-have, we only check it if it is available - it may not always be preserved.
